...
const FEATURES = [
  { icon: <Zap size={20} />, title: "Bulk Pricing", desc: "Tiered wholesale rates on all orders" },
  { icon: <Truck size={20} />, title: "Pan-India Delivery", desc: "Free shipping on orders above ₹5000" },
  { icon: <Shield size={20} />, title: "Verified Sellers", desc: "All sellers are approved & trusted" },
];
      ...
      <section className="relative overflow-hidden bg-steel-950 border-b border-steel-800">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 lg:py-28">
          <div className="max-w-3xl">
            <span className="badge badge-amber mb-4">B2B Marketplace India</span>
            <h1 className="font-display font-800 text-5xl sm:text-6xl lg:text-7xl uppercase leading-none text-steel-50 mb-6">
              Build Smarter.<br />
              <span className="text-amber-400">Buy Bulk.</span>
            </h1>
            <p className="text-steel-400 font-body text-lg mb-8 max-w-xl leading-relaxed">
              Construction materials, hardware tools &amp; agricultural equipment — all in one place. Get wholesale pricing on every order.
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-steel-800">
            {FEATURES.map(({ icon, title, desc }) => (
              <div key={title} className="flex items-center gap-4 px-6 py-5">
                ...
                <div>
                  <p className="font-display font-700 text-sm uppercase tracking-wider text-steel-100">{title}</p>
      ...
      <div className="page-container space-y-16">
        ...
        <section>
          <div className="flex items-end justify-between mb-6">
            <div>
              <p className="text-amber-400/60 font-mono text-xs uppercase tracking-widest mb-1">Shop by</p>
              <h2 className="section-title">Categories</h2>
            </div>
            <Link to="/products" className="text-sm text-steel-400 hover:text-amber-400 font-body flex items-center gap-1 transition-colors">
          ...
          {loading ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {Array(6).fill(0).map((_, i) => <div key={i} className="skeleton h-28 rounded-lg" />)}
            </div>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 stagger">
              {categories.map((cat) => (
                ...
                >
                  <div className="text-3xl mb-3">{cat.icon || CATEGORY_ICONS[cat.name] || "📦"}</div>
        ...
        <section>
          <div className="flex items-end justify-between mb-6">
            <div>
              <p className="text-amber-400/60 font-mono text-xs uppercase tracking-widest mb-1">Handpicked</p>
          ...
          {loading ? (
            ...
            <p className="text-steel-500 font-body text-center py-12">No featured products yet.</p>
          ) : (
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            <div>
              ...
              <p className="text-steel-400 font-body mt-2">
                Join thousands of sellers. List your products and reach bulk buyers across India.
              </p>
            </div>
